...
def start():
	...
	# for testing 
	# scheduler.add_job(reset_daily_stock_sold, 'interval', seconds=60)  
	# scheduler.add_job(reset_weekly_stock_sold, 'interval', minutes=2)  # weekly_stock_sold resets weekly
	# scheduler.add_job(reset_monthly_stock_sold, 'interval', minutes=3)  # monthly_stock_sold resets monthly
	# scheduler.add_job(add_new_data, 'interval', seconds=40)         
	# scheduler.add_job(periodic_model_trainer, 'interval', minutes = 4)   
	scheduler.start()
